            ...
            <AnimatePresence mode="wait">
              {activePulse ? (
                ...
                >
                  ...
                  {/* Arrows pointing toward quadrants */}
                  <div className="mt-8 pt-6 border-t border-white/10">
                    <div className="space-y-3">
                      {pulses.map((pulse, index) => (
                        ...
                        >
                          <div
                            className={`flex-shrink-0 w-10 h-10 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center ${pulse.color} group-hover:bg-white/10 transition-colors`}
                          >
                            {pulse.icon}
                          </div>
                          <span className={`text-sm font-medium flex-1 ${pulse.color} opacity-90 group-hover:opacity-100`} dir={dir}>
                            {pulse.title}
                          </span>
                          <motion.span
                            className={`flex-shrink-0 ${pulse.color}`}
                            animate={{ x: [0, 6, 0] }}
                            transition={{
                              repeat: Infinity,
                              duration: 1.6,
                          ...
                          >
                            <IconArrowRight className="w-5 h-5 hidden lg:block" stroke={2.5} />
                            <IconArrowUp className="w-5 h-5 lg:hidden" stroke={2.5} />
                          </motion.span>
                        </motion.button>
                      ))}
                    </div>
                    ...
                    <motion.div
                      className="mt-5 flex items-center justify-end text-primary-400/80"
                      animate={{ x: [0, 8, 0] }}
                      transition={{ repeat: Infinity, duration: 2, ease: 'easeInOut' }}
                    >
                      <IconArrowRight className="w-6 h-6 hidden lg:block" stroke={2} />
                      <IconArrowUp className="w-6 h-6 lg:hidden" stroke={2} />
                    </motion.div>
                  </div>
                </motion.div>
              )}
