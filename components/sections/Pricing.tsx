      ...
      <div className="container mx-auto px-4">
        ...
        >
          <h2 className="text-4xl md:text-5xl font-bold mb-4 font-tech text-center flex items-center justify-center" dir={dir}>
            <span className="text-white inline-block">{(t?.pricing?.title || 'Flexible Plan').substring(0, Math.ceil((t?.pricing?.title || 'Flexible Plan').length / 2))}</span>
            <span className="text-primary-500 inline-block">{(t?.pricing?.title || 'Flexible Plan').substring(Math.ceil((t?.pricing?.title || 'Flexible Plan').length / 2))}</span>
          </h2>
